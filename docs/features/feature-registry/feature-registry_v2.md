...
When clicking "Product Test Suite" in the LNB, the user navigates step by step through a hierarchical structure.
...
```
Product Test Suite › Acme Corp › Cloud (WEB) › Login › Social Login
```
...
**Entry point**: Click "Product Test Suite" in the LNB
...
<Route path="/features">
  ...
  <Route path="companies/:companyId/products/:productId" element={<FeatureListPage />} />
  <Route path="companies/:companyId/products/:productId/features/:featureId" element={<TestCasePage />} />
</Route>
```

### Global State (ActiveCompanyContext)

```ts
// ActiveCompanyContext.tsx — React Context로 구현됨
...
> `ActiveCompanyContext`가 `activeCompany` 상태를 전역 관리 (Zustand store도 존재하지만 Context가 주력).
...
<Breadcrumb>
  <BreadcrumbItem to="/features">Product Test Suite</BreadcrumbItem>
...
| Phase | Task | Status |
|---|---|---|
| 1 | `V2__create_test_case.sql` Flyway migration | ✅ Done |
| 2 | TestCase Entity / Repository / Service / Controller | ✅ Done |
| 3 | Frontend routing restructure (4-step drill-down) | ✅ Done |
| 4 | Breadcrumb + page component for each step | ✅ Done |
| 5 | TC step builder UI | ✅ Done |
| 6 | AI draft generation API + frontend integration | ✅ Done |
| 7 | My Senior / Ticket Reviewer integration | 🔲 Planned |

### Implementation Notes
- `FloatArrayToVectorConverter`: float[] ↔ pgvector 변환을 위한 JPA AttributeConverter 구현
- Embedding 비활성화 설정: `feature.embedding.enabled=false`로 임베딩 생성 건너뛰기 가능
- AI draft generation: ChatClient(Claude)를 사용하여 Feature의 description + promptText 기반 TC step 자동 생성