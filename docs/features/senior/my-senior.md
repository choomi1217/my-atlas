...
**FAQ 큐레이션 알고리즘** — 자동으로 가장 유용한 지식을 선별:

| 구분 | 조건 | 최대 수 | QA 의미 |
|------|------|---------|---------|
| **고정(Pinned)** | 관리자가 KB에서 Pin 설정 | 15건 | 반드시 알아야 할 핵심 QA 지식 |
| **자주 검색됨** | Chat RAG에서 조회 빈도(hit_count) 상위 | 5건 | 팀이 실제로 많이 찾는 실무 지식 |
| **합계** | 중복 제거 후 병합 | **최대 20건** | 한 화면에서 핵심 지식 파악 가능 |
...
| 기능 | 설명 | QA 의미 |
|------|------|---------|
| **Pin (고정)** | 최대 15건까지 FAQ에 고정 노출 | 신입 QA가 반드시 읽어야 할 핵심 지식 고정 |
...
| GET | `/api/senior/faq` | 큐레이션된 FAQ 목록 (고정 15 + 조회수 Top 5, 최대 20건) |
