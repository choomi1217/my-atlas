...
| 워크플로우 | Slack 알림 | 조건 |
|-----------|-----------|------|
| `frontend-ci.yml` | O | 실패 시만 |
| `backend-ci.yml` | O | 실패 시만 |
| `e2e.yml` | O | 실패 시만 |
| `deploy-frontend.yml` | X | 없음 |
| `deploy-backend.yml` | X | 없음 |

### 변경 후

| 워크플로우 | Slack 알림 | 조건 |
|-----------|-----------|------|
| `frontend-ci.yml` | O | **성공 + 실패 모두** |
| `backend-ci.yml` | O | **성공 + 실패 모두** |
| `e2e.yml` | O | **성공 + 실패 모두** |
| `deploy-frontend.yml` | O | **성공 + 실패 모두** |
| `deploy-backend.yml` | O | **성공 + 실패 모두** |
...
2. PR 생성하여 CI 워크플로우 트리거 → 성공 시 Passed 알림, 실패 시 Failed 알림 확인
